        ...
        if 'enrollment' in request.POST:
            try:
                ...
                for x in students:
                    ...
                    addcourse.save()
                messages.success(request, 'Course enrollment successful.',extra_tags='success')
            except Exception as e:
                messages.error(request, 'An error occurred while enrolling the data. Please try again later.',extra_tags='danger')
                print(e)
...
def admins_dashboard(request):
    batch=student.objects.values('batch').distinct()
    degree=student.objects.values('degree').distinct()
    branch=student.objects.values('branch').distinct()
    year=student.objects.values('year').distinct()
    semester=student.objects.values('semester').distinct()
    
    return render(request, 'admin/dashboard.html',{"batch":batch,"degree":degree,"branch":branch,"year":year,"semester":semester}) 


@user_passes_test(staff_text)
def admins_profile_student(request,username):
    ...
    if 'change_username' in request.POST:
        try:
            ...
            return redirect('admins_profile_student',rolls)
        except Exception as e:
    ...
    if 'update_record' in request.POST:
        try:
            ...
            students.save()
            messages.success(request, 'Record updated successfully.', extra_tags='success')
            return redirect('admins_profile_student',username)
        except Exception as e:
            messages.error(request, 'An error occurred while updating the record. Please try again later.', extra_tags='danger')
            print(e)  
    ...
    try:
        ...
        courses = enrollments.courses.all()
    except enrollment.DoesNotExist:
        return render(request, 'admin/profile_student.html',{"student_profile":student_profile}) 
    return render(request, 'admin/profile_student.html',{"student_profile":student_profile,"courses":courses}) 

@user_passes_test(staff_text)
def admins_profile_teacher(request,username):
    user = User.objects.get(username=username)
    teacher_profile = employee.objects.filter(user=user).get()
    data1 = teacher_profile.roll
    if data1!=username:
        teacher_profile.roll=username
        teacher_profile.save()
    if 'reset_password' in request.POST:
            try:
                user = User.objects.get(username=username)
                dates=teacher_profile.dob.strftime('%d-%m-%Y')
                user.set_password(str(dates))
                user.save()
                messages.success(request, 'Reset Password successful.',extra_tags='success')
            except Exception as e:
    ...
    if 'change_username' in request.POST:
        try:
            ...
            users = User.objects.get(username=username)
            if request.FILES.get('studentimage'):
                teacher_profile = employee.objects.filter(user=users).get()
                teacher_profile.photo =  request.FILES.get('studentimage')
                teacher_profile.save()
            users.username = rolls
            users.save()
            messages.success(request, 'Record updated successfully.', extra_tags='success')
            return redirect('admins_profile_teacher',rolls)
        except Exception as e:
            messages.error(request, 'An error occurred while updating the record. Please try again later.', extra_tags='danger')
            print(e)
    if 'update_record' in request.POST:
        try:
            user = User.objects.get(username=username)
            employees = employee.objects.filter(user=user).get()
            employees.name  = request.POST['name']
            employees.mobile  = request.POST['mobile']
            employees.dob  = request.POST['dob']
            employees.email  = request.POST['email']
            employees.department  = request.POST['department']
            employees.designation  = request.POST['designation']
            employees.coordinate  = request.POST['coordinate']
            employees.save()
            messages.success(request, 'Record updated successfully.', extra_tags='success')
            return redirect('admins_profile_teacher',username)
        except Exception as e:
            messages.error(request, 'An error occurred while updating the record. Please try again later.', extra_tags='danger')
            print(e)  
    if 'delete_user' in request.POST:
        try:
            ...
            messages.error(request, 'An error occurred while updating the record. Please try again later.', extra_tags='danger')
            print(e)  
    return render(request, 'admin/profile_teacher.html',{"teacher_profile":teacher_profile}) 
    ...
    if request.method == 'POST':
        if 'update_attendance' in request.POST:
            try:
                ...
                student_profile = student.objects.filter(roll=roll).get()
                person = attendance_record.objects.filter(user=student_profile, date=date).get()
                person.date = date
...
def attendance_student(request,username):
    profile=get_user_model().objects.filter(username=username).get()
    student_data=student.objects.filter(roll=profile).get()
    attendances = attendance_record.objects.filter(user=student_data).all()
    enrollments = enrollment.objects.filter(user=student_data).get()
    holiday = holidays.objects.all()
    courses = enrollments.courses.all()
    data1 = []
    for attendance in attendances:
        date_string = attendance.date.strftime('%d-%m-%Y')
       
        attendance_data = {
            'id': attendance.id,
            'user': attendance.user.roll,
            'date': date_string,
           
            'status': attendance.status,
        }
        data1.append(attendance_data)
    data2=[]
    for x in holiday:
        date_holiday = x.date.strftime('%d-%m-%Y')
        holiday_data = {
            'id' : x.id,
            'date': date_holiday,
            'reason': x.reason,
        }
        data2.append(holiday_data)
    data3=[]
    for y in courses:
        course_datas = course_period.objects.filter(name=y).get()
        start_date = course_datas.start.strftime('%d-%m-%Y')
        end_date = course_datas.end.strftime('%d-%m-%Y')
        course_data = {
            'name':course_datas.name,
            'semester':course_datas.semester,
            'start': start_date,
            'end': end_date,
        }
        data3.append(course_data)
    return render(request, 'student/attendance_student.html',{"profile":student_data,'attendances': json.dumps(data1),'holidays': json.dumps(data2),'courses': json.dumps(data3)})  
